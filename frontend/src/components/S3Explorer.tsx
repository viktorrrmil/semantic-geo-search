...
export default function S3Explorer({
    ...
    const selectedFilePath = selectedFile?.type === 'file' ? selectedFile.path : null
    const selectedFolderPath = selectedFile?.type === 'folder' ? selectedFile.path : null
    ...
    function handleFileClick(entry: S3Entry) {
        ...
        if (selectedFile?.type === 'file' && selectedFilePath === filePath) {
            onFileSelect(null)
        } else {
            onFileSelect({type: 'file', path: filePath, name: entryDisplayName(entry.key), region})
        }
    }

    function handleSelectAllFiles() {
        if (!currentPath || fileEntries.length === 0) return
        const folderPath = `s3://${currentPath}`
        ...
        const files: FileItem[] = fileEntries.map(entry => ({
            ...
            name: entryDisplayName(entry.key),
            region,
        }))
    ...
    const openState = isDrawer ? isOpen : true

    return (
        <>
            <div
                className={`${isDrawer ? 'fixed top-0 left-0 h-full w-[360px] shadow-xl z-50' : 'relative h-full w-full'} bg-white border-r border-gray-200 flex flex-col transition-transform duration-200 ease-in-out ${
            ...
            >
                ...
                {breadcrumbs.length > 0 && (
                    <div
                        className="px-4 py-2 border-b border-gray-100 shrink-0 flex flex-wrap gap-x-0.5 gap-y-1 items-center min-h-[34px]">
                ...
                {/* Index-all button */}
                {showIndexActions && fileEntries.length > 0 && !loading && (
                ...
                {fileEntries.length > 0 && !loading && (
                    <div className="px-4 py-2 border-b border-gray-100 shrink-0">
                        <button
                            ...
                            className={`text-[11px] border px-3 py-1.5 rounded-md transition-colors w-full text-left ${
                                selectedFolderPath === `s3://${currentPath}`
                                    ? 'text-emerald-700 bg-emerald-50 border-emerald-200'
                                    : 'text-gray-600 bg-white border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                            }`}
                        >
                            {selectedFolderPath === `s3://${currentPath}`
                                ? `✓ All ${fileEntries.length} files selected`
                                : `Select all ${fileEntries.length} files in this folder`}
                        </button>
                    </div>
                )}
                    ...
                    {loading && (
                        <div className="flex items-center justify-center py-12">
                            <div className="flex gap-1.5">
                                <span
                                    className="w-1.5 h-1.5 rounded-full bg-gray-300 animate-bounce [animation-delay:-0.3s]"/>
                                <span
                                    className="w-1.5 h-1.5 rounded-full bg-gray-300 animate-bounce [animation-delay:-0.15s]"/>
                    ...
                    {!loading && !error && !currentPath && (
                        <div className="flex items-center justify-center py-12 px-6 text-center">
                            <p className="text-xs text-gray-400 leading-relaxed">
                                Enter an S3 path above and press <span
                                className="font-medium text-gray-500">Go</span> to browse
                            </p>
                        </div>
                    )}
                        ...
                        <ul className="py-1.5">
                            {entries.map(entry => (
                                <EntryRow
                                    ...
                                    onIndex={showIndexActions ? () => handleFileIndex(entry) : undefined}
                                    showIndexAction={showIndexActions}
                                />
                            ))}
                        </ul>
                    )}
...
function EntryRow({
    ...
    return (
        ...
        {isFolder ? (
            ...
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <rect x="2" y="1" width="8" height="12" rx="1" fill="currentColor" fillOpacity=".15"
            ...
            <div className="relative bg-white rounded-lg shadow-2xl border border-gray-200 w-[360px] p-5 z-10 mx-4">
                ...
                {target.isFolder && (
                    <p className="text-xs text-gray-500 mb-3">
                        Found <span
                        className="font-medium text-gray-700">{target.fileCount}</span> file{target.fileCount !== 1 ? 's' : ''} in
                        this folder.
                    </p>
                )}
                ...
                {!count && (
                    <p className="text-[11px] text-gray-400 mb-3 -mt-2">Leaving empty will index all available
                        items.</p>
                )}
